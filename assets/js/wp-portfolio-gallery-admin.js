(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.-upgrade-popup').fadeIn(200);
    });
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTWPFOLIOCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jlt_wp_portfolio_recommended_activate_plugin",
            file: file,
            nonce: JLTWPFOLIOCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jlt_wp_portfolio_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTWPFOLIOCORE.recommended_nonce
        },
