...
use JLTWPFOLIO\JLT_WP_Portfolio;
...
if ( ! class_exists( 'Upgrader' ) ) {
	...
	class Upgrader {
		...
		public function __construct() {
			$this->option_name = JLT_WP_Portfolio::plugin_version_key();
		}
		...
		protected function get_installed_version() {
			return get_option( $this->option_name, JLTWPFOLIO_VER );
		}
		...
		public function if_updates_available() {
			if ( version_compare( $this->get_installed_version(), JLTWPFOLIO_VER, '<' ) ) {
				return true;
			}
		...
		public function run_updates() {
			...
			$path              = trailingslashit( JLTWPFOLIO_DIR );
