...
if ( ! class_exists( 'Helper' ) ) {
	...
	class Helper {
		 ...
		 */
		public static function jlt_wp_portfolio_slug_cleanup() {
			return str_replace( '-', '_', strtolower( JLTWPFOLIO_SLUG ) );
		}
		 ...
		 */
		public static function jlt_wp_portfolio_current_datetime() {
			if ( function_exists( 'current_datetime' ) ) {
				return current_datetime();
			}

			return new \DateTimeImmutable( 'now', self::jlt_wp_portfolio_wp_timezone() );
		}

		/**
		 * Function jlt_wp_portfolio_wp_timezone() compability for wp version < 5.3
		 *
		 * @return DateTimeZone
		 */
		public static function jlt_wp_portfolio_wp_timezone() {
			if ( function_exists( 'wp_timezone' ) ) {
				return wp_timezone();
			}

			return new \DateTimeZone( self::jlt_wp_portfolio_wp_timezone_string() );
		}
		...
		public static function api_endpoint() {
			...
			$api_endpoint     = apply_filters( 'jlt_wp_portfolio_endpoint', $api_endpoint_url );
		...
		public static function crm_endpoint() {
			...
			$crm_endpoint     = apply_filters( 'jlt_wp_portfolio_crm_crm_endpoint', $crm_endpoint_url );
		...
		public static function crm_survey_endpoint() {
			...
			$crm_feedback_endpoint     = apply_filters( 'jlt_wp_portfolio_crm_crm_endpoint', $crm_feedback_endpoint_url );

			return trailingslashit( $crm_feedback_endpoint );
		}

		/**
		 * Function jlt_wp_portfolio_wp_timezone_string() compability for wp version < 5.3
		 *
		 * @return string
		 */
		public static function jlt_wp_portfolio_wp_timezone_string() {
		...
		public static function get_merged_data( $data, $start_date = '', $end_data = '' ) {
			$_data = shortcode_atts(
				array(
					'image_url'        => JLTWPFOLIO_IMAGES . '/promo-image.png',
			...
			if ( empty( $_data['image_url'] ) ) {
				$_data['image_url'] = JLTWPFOLIO_IMAGES . '/promo-image.png';
			}
