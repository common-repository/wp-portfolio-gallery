...
class Notifications {
	...
	public function __construct() {
		...
		add_action( 'jlt_wp_portfolio_display_notice', array( $this, 'display_notice' ), 10, 2 );
		add_action( 'jlt_wp_portfolio_display_popup', array( $this, 'display_popup' ), 10, 2 );

		add_action( 'wp_ajax_jlt_wp_portfolio_notification_action', array( $this, 'notification_action' ) );
	}
	...
	public function notification_action() {
		check_ajax_referer( 'jlt_wp_portfolio_notification_nonce' );
	...
	public function setup_notifications_by_type( $type ) {
		...
		// Block if necessary .
		$notification_last_fired = get_option( "jlt_wp_portfolio_{$type}_last_interact" );
		...
		do_action( "jlt_wp_portfolio_display_{$type}", $notification, $trigger_time );
	}
	...
	public function display_popup( $popup, $trigger_time ) {
			...
			<div class="-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); ---popup-color: <?php echo esc_attr( $popup->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="-popup-modal-footer">
					...
					<div class="-popup-countdown" style="display: none;">
						<span class="-popup-countdown-text"><?php echo esc_html__( 'Deal Ends In', 'wp-portfolio-gallery' ); ?></span>
						<div class="-popup-countdown-time">
							<div>
								<span data-counter="days">00</span>
								<span><?php echo esc_html__( 'Days', 'wp-portfolio-gallery' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="hours">00</span>
								<span><?php echo esc_html__( 'Hours', 'wp-portfolio-gallery' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="minutes">00</span>
								<span><?php echo esc_html__( 'Minutes', 'wp-portfolio-gallery' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="seconds">00</span>
								<span><?php echo esc_html__( 'Seconds', 'wp-portfolio-gallery' ); ?></span>
							</div>
						</div>
					</div>
		...
		<script>
			function jlt_wp_portfolio_popup_action(evt, $this, $action_type) {
				...
				jQuery.post('<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>', {
					action: 'jlt_wp_portfolio_notification_action',
					_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jlt_wp_portfolio_notification_nonce' ) ); ?>',
			...
			jQuery('body').on('click', '.-popup .popup-dismiss', function(evt) {
				jlt_wp_portfolio_popup_action(evt, jQuery(this), 'dismiss');
			});

			// Notice Disable
			jQuery('body').on('click', '.-popup .popup-disable', function(evt) {
				jlt_wp_portfolio_popup_action(evt, jQuery(this), 'disable');
			});
		</script>
