...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jlt_wp_portfolio_subscribe', array( $this, 'jlt_wp_portfolio_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jlt_wp_portfolio_subscribe() {
			check_ajax_referer( 'jlt_wp_portfolio_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice- is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> -notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss -notice-dismiss"></button>
				<img width="70" height="70" src="<?php echo esc_url( JLTWPFOLIO_IMAGES . '/logo.png' ); ?>" alt="<?php esc_attr_e( 'Logo', 'wp-portfolio-gallery' ); ?>">
				<div class="-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex">
						<div class="-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary -subscribe-btn"><?php esc_html_e( 'Get Discount', 'wp-portfolio-gallery' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.-notice-subscribe .-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jlt_wp_portfolio_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jlt_wp_portfolio_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								jlt_wp_portfolio_notice_action(null, form, 'disable');
