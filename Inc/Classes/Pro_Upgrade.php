...
use JLTWPFOLIO\Inc\Classes\Notifications\Base\Date;
...
class Pro_Upgrade {
	...
	public function __construct() {
		$this->slug = Helper::jlt_wp_portfolio_slug_cleanup();
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jlt_wp_portfolio_dashboard_widget',                          // Widget slug.
			esc_html__( 'WP Portfolio Gallery News & Updates', 'wp-portfolio-gallery' ), // Title.
		...
		$example_widget_backup = array( 'jlt_wp_portfolio_dashboard_widget' => $default_dashboard['jlt_wp_portfolio_dashboard_widget'] );
		unset( $default_dashboard['jlt_wp_portfolio_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>
			...
			<div class="-dashboard-promo" style="---popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="WP Portfolio Gallery Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'wp-portfolio-gallery' );
		...
		<div class="-dashboard_footer">
			<ul>
				<li class="-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'wp-portfolio-gallery' ); ?>
                        </span>
		...
		<style>
			...
			.-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.-rss-widget .-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.-rss-widget .-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.-rss-widget .-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.-rss-widget .-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.-rss-widget .-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jlt_wp_portfolio_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jlt_wp_portfolio_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jlt_wp_portfolio_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTWPFOLIO_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jlt_wp_portfolio_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jlt_wp_portfolio_sheet_promo_data', $remote_data );
			update_option( 'jlt_wp_portfolio_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jlt_wp_portfolio_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jlt_wp_portfolio_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="-popup -upgrade-popup" id="-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="-popup-overlay"></div>

			<div class="-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); ---popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="-popup-modal-footer">
					...
					<div class="-popup-countdown" style="display: none;">
						<span class="-popup-countdown-text">Deal Ends In</span>
					...
					<!-- button  -->
					<a class="-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>
		...
		<script>
			...
			function updateCounter(seconds) {
				const $counter = $container.find(".-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".-popup-countdown").hide(0);
				}
